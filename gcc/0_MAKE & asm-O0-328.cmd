...
set TARGET="atmega328"
::set TARGET="attiny13"

:: компилировать
::@%AVRBINPATH%\avr-gcc.exe -Wall -Os -mmcu=attiny13 -c "%~n1.cpp" -o "./Release/%~n1.o"
@%AVRBINPATH%\avr-gcc.exe -Wall -O0 -mmcu=%TARGET% -c "%~n1.cpp" -o "./Release/%~n1.o"

:: линкер
@%AVRBINPATH%\avr-gcc.exe -Wall -Os -mmcu=%TARGET% -o "./Release/%~n1.elf" "./Release/%~n1.o"

:: hex файл
@%AVRBINPATH%\avr-objcopy.exe -j .text -j .data -O ihex "./Release/%~n1.elf" "./Release/%~n1.hex"

:: показать размер - что получилось то
@%AVRBINPATH%\avr-size.exe --format=avr --mcu=%TARGET% "./Release/%~n1.elf"
